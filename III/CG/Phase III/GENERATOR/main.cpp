    ...
    for (i = 0; i < numc; i++) {
        for (j = 0; j <= numt; j++) {
            for (k = 1; k >= 0; k--) {
                ...
                res = res + to_string(x) + "," + to_string(y) + "," + to_string(z) + "\n";
            }
        }
    }
...
vector<Point> buildPointsComet(vector<vector<vector<Point>>> patches, int tessellation) {
    ...
    vector<vector<Point>> grid = initGrid(tessellation);
    ...
    for(int i = 0; i<patches.size(); i++) {
        ...
        for (int j = 0; j <= tessellation; j++) {
            ...
            for (int k = 0; k <= tessellation; k++) {
                ...
                grid[j][k].setX(final.getX());
                grid[j][k].setY(final.getY());
                grid[j][k].setZ(final.getZ());
            }
        }
...
bool creatComet(vector<string> params) {
    ...
    vector<Point> coordenates = buildPointsComet(patches,tessellation);
    string res;

    res = to_string(coordenates.size())+"\n";

    for(int i = 0; i < coordenates.size(); i++){
        Point p = coordenates[i];
